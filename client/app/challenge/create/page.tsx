...
export default function Page() {
  ...
  const handleNewChallengeCreation = async () => {
    setIsPending(true);
    try {
      const loggedInUser = JSON.parse(localStorage.getItem(keys.user) ?? "{}");
      if (!loggedInUser) {
    ...
    <CenteredGrid>
      <section className="w-screen md:w-[512px] max-w-lg flex flex-col items-center py-2 px-4 relative">
        <section className="text-center flex flex-col mt-8">
          <h2 className="font-bold mb-2 text-2xl">Create a Challenge</h2>
          <p className="text-faded">A unique game link will be generated.</p>
        </section>
        {/* TIME CONTROL */}
