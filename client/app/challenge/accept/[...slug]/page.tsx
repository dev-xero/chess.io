...
export default function Page() {
  ...
  const acceptChallenge = useCallback(async (gameId: string) => {
    ...
    try {
      ...
      localStorage.setItem(keys.game.active, JSON.stringify(payload.gameState));
      ...
      if (axiosError.response) {
        ...
        const error =
          ((err as AxiosError).response?.data as ErrorResponse)?.["message"] ??
        ...
        <section className="w-screen md:w-[512px] max-w-lg flex flex-col items-center py-2 px-4 relative">
          <section className="text-center flex flex-col mt-8">
            ...
            <p className="text-faded mt-4  my-8">
              <b className="text-foreground">{pathname.split("/")[3]}</b> is
              challenging you to a chess game.
              <br /> May the best player win.
            </p>
            <FilledButton
