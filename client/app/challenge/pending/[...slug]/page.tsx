...
export default function Page() {
  ...
  const { sendJsonMessage, lastMessage, readyState } = useWebSocket(config.ws, {
    share: false,
        ...
        {isMounted && (
          <section className="w-screen md:w-[512px] max-w-lg flex flex-col items-center py-2 px-4 relative">
            <section className="text-center flex flex-col mt-8">
              <h2 className="font-bold mb-2 text-2xl">Challenge Pending</h2>
              <p className="text-faded">
                Waiting for someone to accept this challenge. Refresh this page
              ...
              {pendingChallenge && (
                <>
                  <p
                    ...
                    onClick={async () => await handleLinkShare(challengeLink)}
                  >
                    copy challenge link
                  </p>
                  <p className="mt-6 text-faded text-xs">
                    This challenge will expire in {pendingChallenge.expiresIn}
                  </p>
                </>
              )}
            </section>
