...
import { BoardMove } from "@/interfaces/GameInterface";
...
export default function ClickableChessboard(props: IChessBoardInterface) {
  const [gameInstance, setGameInstance] = useState<Chess>(new Chess(props.fen));
  ...
  const getMoveOptions = useCallback(
    (square: Square) => {
      ...
      moves.forEach((move: Move) => {
        ...
        const isCapture =
          targetPiece && targetPiece.color !== gameInstance.get(square)?.color;
  ...
  const onPromotionPieceSelect = useCallback(
    (piece?: PromotionPieceOption) => {
      if (piece && moveFrom && moveTo) {
        makeMove(moveFrom, moveTo, piece[1].toLowerCase() as PieceSymbol);
      }
      setMoveFrom("");
      setMoveTo(null);
