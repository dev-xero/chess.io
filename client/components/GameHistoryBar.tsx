        ...
        <ul className="flex md:flex-col gap-2 md:h-[360px] max-h-[360px] overflow-x-auto md:overflow-y-auto py-2">
          {props.moveHistoryPairs.length > 0 ? (
            props.moveHistoryPairs.map((movePair, idx) => {
              return (
                <li key={idx} className="md:grid grid-cols-5">
                  <span className="col-span-1 text-primary">{idx + 1}.</span>
                  <span className="col-span-2 font-bold">{movePair[0]}</span>
        ...
        <h3 className="flex items-center gap-2">
          ...
          <div>
            <p
              className={clsx(
                "font-bold text-xl",
                props.whoseTurn == "w" ? "text-white" : "text-foreground",
              )}
            >
              {props.whoseTurn == "w" ? "White" : "Black"}&apos;s Turn
            </p>
